...
export function MobileSubscriptionsContent() {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (refreshResult.success)
          setData(refreshResult.data as SubscriptionData);
      } else {
      ...
      <div className="mb-5 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            Akses semua tryout dalam satu paket
          </p>
        </div>
      </div>
          ...
          <Card className="border-2 border-emerald-500/50 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20">
            ...
            <CardContent className="space-y-2">
              {data.userSubscriptions.map((sub) => (
                <div
                  key={sub.id}
                  className="flex items-center justify-between rounded-lg bg-white/60 p-3 dark:bg-background/30"
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {sub.plan} · Berlaku hingga{" "}
                      {new Date(sub.endDate).toLocaleDateString("id-ID")}
                    </p>
                  </div>
                  <Badge variant="default" className="shrink-0 bg-emerald-500">
                    Aktif
                  </Badge>
                </div>
              ))}
        ...
        <div className="flex gap-1 rounded-xl bg-muted/60 p-1">
          {(
            ...
            <button
              ...
              className={cn(
                "relative flex-1 rounded-lg px-3 py-2.5 text-sm font-medium transition-all min-h-[44px]",
                selectedPlan === key
            ...
            >
              ...
              <CardContent className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full rounded-xl min-h-12"
